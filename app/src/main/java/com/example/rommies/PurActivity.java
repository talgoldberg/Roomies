...
import java.util.Map;
 ...
 public class PurActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        key_ap=(String)getIntent().getExtras().getString("com.app.java.purActivity.key");
        users = (Map<String, String>)getIntent().getExtras().get("com.app.java.purActivity.users");
        mAuth = FirebaseAuth.getInstance();
        FirebaseUser user = mAuth.getCurrentUser();
        userID = user.getUid();
        Money=(EditText)findViewById(R.id.amount);
        btfinish=(Button)findViewById(R.id.finish1);
        mRef = FirebaseDatabase.getInstance().getReference("/Aprtment/"+key_ap+"/Balance").child(userID);
        ...
        ArrayAdapter<String> adapter1 = new ArrayAdapter<String>(PurActivity.this, android.R.layout.simple_spinner_item,paths);
            ...
            btfinish.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    spin=spinner.getSelectedItem().toString();
                     double finalamounet=Amount/(checkedNames.size()+1);
                    ArrayList<String> uid=new ArrayList<>();
                    if(checkedNames.isEmpty()){}

                    aprRef = FirebaseDatabase.getInstance().getReference("Aprtment");
                    for(Map.Entry<String, String> entry:checkedNames.entrySet())
                    {
                            ...
                            double money=(Double)status.get(entry.getKey());
                                ...
                                mRef.child(entry.getKey()).setValue(money + finalamounet);
                            if(money>0){
                                aprRef.child(key_ap).child("Balance").child(entry.getKey()).child(userID).setValue(money-finalamounet);
                            }else{
                                aprRef.child(key_ap).child("Balance").child(entry.getKey()).child(userID).setValue(-money-finalamounet);
                    ...
                    pay = new payment(userID,finalamounet, spin,uid);
